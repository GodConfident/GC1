...
使用说明：
    网页嵌入（IT之家、辣品、IT圈）：
        ...
        4）各网站callback页面：
            IT之家：http://www.ithome.com/ithome/openplat/qq/callBack.aspx
            辣品：
            IT圈：
...
function RegisterUser(userNick, userPsw, userPswConfirm, userQQ, app) {
    ...
    $.ajax({
        ...
        dataType: "json",
        cache: false,
        data: '{mobile:"' + escape(mobile) + '", checkcode:"' + escape(checkcode) + '", nick:"' + escape(userNick) + '", psw:"' + userPsw + '", pswConfirm:"' + userPswConfirm + '", qq:"' + escape(userQQ) + '", avatar:"' + (avatarData == null ? "" : escape(avatarData.substr(0, 32))) + '", app:"' + escape(app) + '"}',
        error: function () {
