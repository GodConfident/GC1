...
<div class="user">
    ...
    <blockquote class="layui-elem-quote">
        <form action="/user" class="layui-form" method="get">
            <button class="layui-btn layui-btn-danger" id='tjyh'>添加单页</button>
        </form>
...
//                    循环数据
                ...
                ,cols: [[ //标题栏
                    ...
                    ,{field: 'url', title: '单页地址', width: 300}
...
//                当点击修改按钮后执行
                if(layEvent=='edit'){
                    layer.open({
                        ...
                        skin: 'layui-layer-rim', //加上边框
                        area: ['420px', '290px'], //宽高
                        content: '<form action="/admin/single/'+data['id']+'"method="post"class="layui-form"style="width: 80%;margin-top: 20px">{{csrf_field()}}<div class="layui-form-item"><label class="layui-form-label">单页标题</label><div class="layui-input-block"><input type="text"name="title"value="'+data['title']+'"class="layui-input"><input type="hidden"name="_method"value="put"></div></div><div class="layui-form-item"><label class="layui-form-label">单页地址</label><div class="layui-input-block"><input type="text"name="url"value="'+data['url']+'"class="layui-input"></div></div><div class="layui-form-item"><label class="layui-form-label">权重设置</label><div class="layui-input-block" style="width:50px"><input type="text"name="rank"value="'+data['rank']+'"class="layui-input"></div></div>{{--提交--}}<div class="layui-form-item"><div class="layui-input-block"><button class="layui-btn"lay-submit lay-filter="formDemo">立即提交</button><button type="reset"class="layui-btn layui-btn-primary">重置</button></div></div></form>'
                    });
                }
//                当点击删除按钮后执行
                if(layEvent=='del'){
                    layer.confirm('您确定要删除数据么？', {
                        ...
                        $.post('/admin/single/'+data['id'],{'_method':'DELETE','_token':'{{csrf_token()}}','id':data['id']},function(msg){
                            if(msg==1){
